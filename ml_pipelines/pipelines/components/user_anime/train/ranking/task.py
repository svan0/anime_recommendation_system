...
from anime_rec.data.data_loaders.train.user_anime import load_user_anime_ranking
from anime_rec.train.user_anime import train_user_anime_ranking_model, evaluate_user_anime_ranking_model

from anime_rec.data.data_loaders.train.user_anime import load_user_anime_list_ranking
from anime_rec.train.user_anime import train_user_anime_list_ranking_model, evaluate_user_anime_list_ranking_model
...
if __name__ == '__main__':
    ...
    if args.model_type == 'ranking':
        train_data = load_user_anime_ranking(train_data_path, data_format, 'tfds')
        ...
        validation_data = load_user_anime_ranking(val_data_path, data_format, 'tfds')
        ...
        test_data = load_user_anime_ranking(test_data_path, data_format, 'tfds')
        ...
        model = train_user_anime_ranking_model(anime_data, user_data, train_data, validation_data, hyperparameters)
        model.save(model_path)

        metrics = evaluate_user_anime_ranking_model(model, train_data, validation_data, test_data)
        ...
        train_data = load_user_anime_list_ranking(train_data_path, data_format, 'tfds')
        train_data = train_data.shuffle(100_000, reshuffle_each_iteration=True)
        train_data = train_data.map(lambda x : (
            {
                'user_id' : x['user_id'],
        ...
        validation_data = load_user_anime_list_ranking(val_data_path, data_format, 'tfds')
        validation_data = validation_data.map(lambda x : (
            {
                'user_id' : x['user_id'],
                'anime_id' : x['anime_id']
        ...
        test_data = load_user_anime_list_ranking(test_data_path, data_format, 'tfds')
        test_data = test_data.map(lambda x : (
            {
                'user_id' : x['user_id'],
                'anime_id' : x['anime_id']
        ...
        model = train_user_anime_list_ranking_model(anime_data, user_data, train_data, validation_data, hyperparameters)
        model.save(model_path)

        metrics = evaluate_user_anime_list_ranking_model(model, train_data, validation_data, test_data)
        metrics_folder = tempfile.mkdtemp()
        with open(f"{metrics_folder}/metrics.json", 'w') as f:
            f.write(json.dumps(metrics, indent=4, sort_keys=True))
        upload_local_folder_to_gcs(metrics_folder, metrics_path)
