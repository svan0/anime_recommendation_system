...
class TaskTest(unittest.TestCase):
    ...
    def test_task_runs(self):
        ...
        results = results[(results['user_id'] == 'xbhrjd') & (results['retrieved_anime_id'] == '18153')]['ranking_score'].values[0]

        model = tf.saved_model.load(model_path)
        results_2 = model({
            'anchor_anime' : tf.constant([['30831']]),
            'rel_anime' : tf.constant([['18153']])
        })
