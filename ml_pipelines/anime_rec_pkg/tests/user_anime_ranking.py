...
from anime_rec.data.data_loaders.train.user_anime import load_user_anime_ranking as train_load_user_anime_ranking
from anime_rec.train.user_anime import get_user_anime_ranking_model, train_user_anime_ranking_model
from anime_rec.data.data_loaders.infer.user_anime import load_user_anime_ranking as infer_load_user_anime_ranking
...
class Test1DataLoadTest(unittest.TestCase):
    ...
    def test_user_anime_list_ranking_dataload(self):
        ...
        data_path = os.path.join(dir_path, 'sample_data/user_anime/ranking/train')

        dataset = train_load_user_anime_ranking(data_path, 'csv', 'tfds').batch(16)
...
class Test2TrainingTest(unittest.TestCase):
    ...
    def test_weights_updated(self):
        ...
        train_data_path = os.path.join(dir_path, 'sample_data/user_anime/ranking/train')

        list_anime = load_list_anime(all_anime_data_path, 'csv', 'numpy')
        list_user = load_list_user(all_user_data_path, 'csv', 'numpy')
        train_dataset = train_load_user_anime_ranking(train_data_path, 'csv', 'tfds').batch(2048)
    ...
    def test_one_batch_overfit(self):
        ...
        all_user_data_path = os.path.join(dir_path, 'sample_data/list_user')
        train_data_path = os.path.join(dir_path, 'sample_data/user_anime/ranking/train')
        validation_data_path = os.path.join(dir_path, 'sample_data/user_anime/ranking/val')

        list_anime = load_list_anime(all_anime_data_path, 'csv', 'numpy')
        list_user = load_list_user(all_user_data_path, 'csv', 'numpy')
        train_dataset = train_load_user_anime_ranking(train_data_path, 'csv', 'tfds').batch(16).take(1)
        train_dataset = train_dataset.map(lambda x : (
            {
                'user_id' : x['user_id'],
                'anime_id' : x['anime_id']
        ...
        val_dataset = train_load_user_anime_ranking(validation_data_path, 'csv', 'tfds').batch(16).take(1)
        ...
        model = train_user_anime_ranking_model(
            list_anime,
            list_user,
            train_dataset,
            val_dataset,
        ...
        model.save(os.path.join(dir_path, 'sample_data/user_anime/ranking/model'))

        train_metrics = model.evaluate(train_dataset, return_dict=True)
        val_metrics = model.evaluate(val_dataset, return_dict=True)

        assert(train_metrics['loss'] < 0.01)
        assert(val_metrics['loss'] > 1000.0 * train_metrics['loss'])
...
class Test3InferenceTest(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        input_data_path = os.path.join(dir_path, 'sample_data/user_anime/ranking/infer')

        infer_data = infer_load_user_anime_ranking(input_data_path, 'csv', 'tfds')
