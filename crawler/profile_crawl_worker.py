...
from utils import run_profile_crawler

def local_crawl_profile(profile_urls):
    crawler_settings = get_project_settings()
    crawler_settings['ITEM_PIPELINES'] = LOCAL_PIPELINES
    run_profile_crawler(profile_urls, crawler_settings)

def cloud_crawl_profile(profile_urls):
    crawler_settings = get_project_settings()
...
if __name__ == '__main__':
    ...
    if args.local:
        load_dotenv()
        crawl_function = local_crawl_profile
    else:
        crawl_function = cloud_crawl_profile
    ...
    with subscriber:
        while True:
            ...
            for received_message in received_messages:
                profile_urls.append(received_message.message.data.decode('utf-8'))
