...
class AnimeProcessPipeline:
    ...
    def process_item(self, item, spider):
        ...
        not_null_fields = [
            'crawl_date', 'uid', 'url', 'title', 'synopsis',
        ...
        for field in not_null_fields:
            if field not in item:
                raise DropItem(f"{item['url']} dropped because '{field}' is null")
        ...
        for field in int_fields:
            if not isinstance(item[field], int):
                raise DropItem(f"{item['url']} dropped because '{field}' is not an integer")
        
        if not isinstance(item['studios'], list):
            raise DropItem(f"{item['url']} dropped because 'studios' is not a list")
        
        if not isinstance(item['genres'], list):
            raise DropItem(f"{item['url']} dropped because 'genres' is not a list")
        
        if ('clubs' in item) and (not isinstance(item['clubs'], list)):
            raise DropItem(f"{item['url']} dropped because 'clubs' is not a list")
        
        if ('pics' in item) and (not isinstance(item['pics'], list)):
            raise DropItem(f"{item['url']} dropped because 'pics' is not a list")
        
        if item['type'] not in {"TV", "Movie", "Special", "OVA", "ONA", "Special"}:
            raise DropItem(f"{item['url']} dropped because 'type' {item['type']} is not known")
        
        if item['source_type'] not in {"Manga", "One-shot", "Doujinshi", "Light novel", "Novel", "Manhwa", "Manhua", "Original"}:
            raise DropItem(f"{item['url']} dropped because 'source_type' {item['source_type']} is not known")
        
        if item['status'] not in {"Not yet aired", "Currently Airing", "Finished Airing"}:
            raise DropItem(f"{item['url']} dropped because 'status' {item['status']} is not known")
        ...
        if item['status'] == "Currently Airing":
            ...
            if 'start_date' not in item:
                raise DropItem(f"{item['url']} dropped because it is airing but 'start_date' is not known")
        
        if item['status'] == "Finished Airing":
            if 'num_episodes' not in item:
                raise DropItem(f"{item['url']} dropped because it has finished airing but 'num_episodes' is not known")
            
            if 'start_date' not in item:
                raise DropItem(f"{item['url']} dropped because it has finished airing but 'start_date' is not known")
            
            if item['num_episodes'] == 1:
                item['end_date'] = item['start_date']
            
            if 'end_date' not in item:
                raise DropItem(f"{item['url']} dropped because it has finished airing but 'end_date' is not known")
        ...
        if item['total_count'] != (item['watching_count'] + item['completed_count'] + item['on_hold_count'] + item['dropped_count'] + item['plan_to_watch_count']):
            raise DropItem(f"{item['url']} dropped because watch 'status_count' do not sum up to 'total_count'")
        ...
        if ('score_count' in item) and (item['score_count'] != sum_score_voters):
            raise DropItem(f"{item['url']} dropped because 'score_xx_count' do not sum up to 'score_count' count")
        ...
        if ('score' in item) and (not isclose(item['score'] , average_score)):
            raise DropItem(f"{item['url']} dropped because computed average score is not close to 'score'")
